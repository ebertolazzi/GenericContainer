...
namespace yy
{
  ...
  inline bool unicode_1( char c )
  {
    return ( reinterpret_cast<unsigned char &>( c ) & 0x80 ) == 0x00;
  }
  inline bool unicode_2( char c )
  {
    return ( reinterpret_cast<unsigned char &>( c ) & 0xE0 ) == 0xC0;
  }
  inline bool unicode_3( char c )
  {
    return ( reinterpret_cast<unsigned char &>( c ) & 0xF0 ) == 0xE0;
  }
  inline bool unicode_4( char c )
  {
    return ( reinterpret_cast<unsigned char &>( c ) & 0xF8 ) == 0xF0;
  }

  inline bool unicode_min_2( char c )
  {
    return ( reinterpret_cast<unsigned char &>( c ) & 0xC0 ) == 0xC0;
  }
  inline bool unicode_min_3( char c )
  {
    return ( reinterpret_cast<unsigned char &>( c ) & 0xE0 ) == 0xE0;
  }
  inline bool unicode_min_4( char c )
  {
    return ( reinterpret_cast<unsigned char &>( c ) & 0xF0 ) == 0xF0;
  }

  inline bool ok_unicode( char c )
  {
    return ( reinterpret_cast<unsigned char &>( c ) & 0xC0 ) == 0x80;
  }

  // Restituisce il prossimo token
  int jsonLexer::yylex( std::string * yylval, yy::location * yylloc )
  {
    ...
    {
      case '{':
        res = yy::parser::token::LEFT_CURLY;
        ++m_cursor;
        break;
      case '}':
        res = yy::parser::token::RIGHT_CURLY;
        ++m_cursor;
        break;
      case '[':
        res = yy::parser::token::LEFT_BRACE;
        ++m_cursor;
        break;
      case ']':
        res = yy::parser::token::RIGHT_BRACE;
        ++m_cursor;
        break;
      case ',':
        res = yy::parser::token::COMMA;
        ++m_cursor;
        break;
      case ':':
        res = yy::parser::token::COLON;
        ++m_cursor;
        break;
      case 't':  // true
