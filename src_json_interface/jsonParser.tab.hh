...
#ifndef YY_YY_JSONPARSER_TAB_HH_INCLUDED
#define YY_YY_JSONPARSER_TAB_HH_INCLUDED
...
public:
  GenericContainerWrapper( GenericContainer * gc ) : m_data( gc )
  {
    m_stack.reserve( 100 );
  ...
  void set_integer( string const & s )
  {
    ...
    if ( *end == '\0' )
    {
      if ( res <= std::numeric_limits<GC::int_type>::max() && res >= std::numeric_limits<GC::int_type>::min() )
      {
  ...
  void open_map()
  {
    m_stack.push_back( get_gc() );
    m_stack.back()->set_map();
  }
...
#pragma GCC message "bison: do not #define YYSTYPE in C++, use %define api.value.type"
    ...
    struct token
    {
      enum token_kind_type
      {
        ...
        LEFT_CURLY  = 258,  // LEFT_CURLY
        RIGHT_CURLY = 259,  // RIGHT_CURLY
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_LEFT_CURLY  = 3,   // LEFT_CURLY
        S_RIGHT_CURLY = 4,   // RIGHT_CURLY
        S_LEFT_BRACE  = 5,   // LEFT_BRACE
        S_RIGHT_BRACE = 6,   // RIGHT_BRACE
        S_COMMA       = 7,   // COMMA
        S_COLON       = 8,   // COLON
        S_VTRUE       = 9,   // VTRUE
        S_VFALSE      = 10,  // VFALSE
    ...
    parser( jsonLexer & lexer_yyarg, GenericContainerWrapper * gcw_yyarg );
    virtual ~parser();
...
#endif  // !YY_YY_JSONPARSER_TAB_HH_INCLUDED
