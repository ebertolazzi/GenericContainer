...
public:
  ...
  string GetToken() const { return m_token; }
...
static
vector<string>
split_string( string const & str, string const & sep ) {
  Tokenizer tk( str, sep );
  vector<string> arr;
  while ( tk.NextToken() ) arr.push_back(tk.GetToken());
  return arr;
}

static
bool
is_string_a_real_number( string const & str_input, double & number ) {
...
static
bool
is_string_an_imaginary_number(
  string const & str_input,
  double       & imaginary_part,
  string const & im_unit
) {
  if ( str_input.empty() ) return false;
  if ( str_input.find( im_unit ) == std::string::npos ) return false;
  ...
  return is_string_a_real_number( str, imaginary_part );
}

static
bool
is_string_a_complex_number(
  string const & str_input,
  ...
  if ( str_input.find( "+" ) == std::string::npos ) {
    ...
    double num;
    if ( is_string_a_real_number( str_input, num ) ) {
      out_number = num;
      return true;
    }
    if ( is_string_an_imaginary_number( str_input, num, im_unit ) ) {
  ...
  vector<string> real_imag = split_string( str_input, "+" );
  if ( real_imag.size() != 2 ) {
    return false;
  }
  double real, imag;
...
static
TypeAllowed
find_minimum_common_type_allowed( TypeAllowed type1, TypeAllowed type2 ) {
...
static
void
convert_GenericVector_to_BoolVector( GenericContainer & root ) {
  vector<bool> new_vec;
...
static
void
convert_GenericVector_to_StringVector(
...
static
void
convert_GenericVector_to_IntVector( GenericContainer & root ) {
...
static
void
convert_GenericVector_to_RealVector( GenericContainer & root ) {
...
static
void
convert_GenericVector_to_ComplexVector( GenericContainer & root ) {
...
GenericContainerJsonHandler::String(
  ...
  if ( stringa_lower.length() != stringa.length() ||
       stringa_lower.compare( "null" ) != 0 ) {
    // check if it is a complex number
    complex_type complex;
    if ( is_string_a_complex_number( stringa, complex, _im_unit ) ) {
