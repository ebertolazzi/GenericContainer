...
namespace GC_namespace
{
  ...
  int GenericContainerExplorer::check( GC_type const data_type ) const
  {
  ...
  int GenericContainerExplorer::check_no_data( GC_type const data_type ) const
  {
    if ( head.empty() ) return GENERIC_CONTAINER_BAD_HEAD;
    if ( GC_type::NOTYPE == head.back()->get_type() || data_type == head.back()->get_type() )
      return GENERIC_CONTAINER_OK;
    return GENERIC_CONTAINER_NOT_EMPTY;
  }

  int GenericContainerExplorer::pop()
  {
  ...
  int GenericContainerExplorer::push( GenericContainer * gc )
  {
    head.push_back( gc );
    return GENERIC_CONTAINER_OK;
  }

  int GenericContainerExplorer::push_vector_position( unsigned const pos )
  {
  ...
  int GenericContainerExplorer::push_map_position( char const pos[] )
  {
  ...
  int GenericContainerExplorer::init_map_key()
  {
    int const ok{ check( GC_type::MAP ) };
    if ( ok == GENERIC_CONTAINER_OK )
    {
  ...
  string_view GenericContainerExplorer::next_map_key()
  {
    if ( map_iterator != ptr_map->end() ) return map_iterator++->first;
    return "";
  }

  int GenericContainerExplorer::reset()
  {
    if ( head.empty() ) return GENERIC_CONTAINER_NO_DATA;
...
extern "C"
{
  ...
  int GC_fill_for_test( char const id[] )
  {
    // ckeck if exists
    GC_select( id );
    GenericContainer & gc = *static_cast<GenericContainer *>( gc_active->mem_ptr() );
  ...
  int GC_get_bool_at_pos( int const pos )
  {
    if ( gc_active == nullptr ) return 0;
  ...
  int GC_get_int_at_pos( int const pos )
  {
    if ( gc_active == nullptr ) return 0;
    return gc_active->top()->get_int_at( static_cast<unsigned>( pos ) );
  }

  double GC_get_real_at_pos( int const pos )
  {
    if ( gc_active == nullptr ) return 0;
    return gc_active->top()->get_number_at( static_cast<unsigned>( pos ) );
  }

  c_complex_type GC_get_complex_at_pos( int const pos )
  {
    c_complex_type tmp;
    tmp.real = tmp.imag = 0;
    if ( gc_active != nullptr )
      gc_active->top()->get_complex_number_at( static_cast<unsigned>( pos ), tmp.real, tmp.imag );
    return tmp;
  }

  double GC_get_complex_real_at_pos( int const pos )
  {
    real_type re = 0, im = 0;
  ...
  double GC_get_complex_imag_at_pos( int const pos )
  {
    real_type re = 0, im = 0;
    if ( gc_active != nullptr ) gc_active->top()->get_complex_number_at( static_cast<unsigned>( pos ), re, im );
    return im;
  ...
  char const * GC_get_string_at_pos( int const pos )
  {
    if ( gc_active == nullptr ) return nullptr;
  ...
  double GC_get_real_at_coor( int const i, int const j )
  {
    if ( gc_active == nullptr ) return 0;
    return gc_active->top()->get_real_at( static_cast<unsigned>( i ), static_cast<unsigned>( j ) );
  }

  c_complex_type GC_get_complex_at_coor( int const i, int const j )
  {
    c_complex_type tmp;
    tmp.real = tmp.imag = 0;
    if ( gc_active != nullptr )
    {
      complex_type const res{
        gc_active->top()->get_complex_at( static_cast<unsigned>( i ), static_cast<unsigned>( j ) )
      };
      tmp.real = res.real();
      tmp.imag = res.imag();
    }
  ...
  double GC_get_complex_real_at_coor( int const i, int const j )
  {
    if ( gc_active != nullptr )
    {
      complex_type const res{
  ...
  double GC_get_complex_imag_at_coor( int const i, int const j )
  {
    if ( gc_active != nullptr )
    {
      complex_type const res{
  ...
  int GC_set_vector_of_bool( int const * a, int const nelem )
  {
    if ( gc_active == nullptr ) return GENERIC_CONTAINER_BAD_HEAD;
  ...
  int GC_set_vector_of_int( int const * a, int const nelem )
  {
    if ( gc_active == nullptr ) return GENERIC_CONTAINER_BAD_HEAD;
  ...
  int GC_set_vector_of_real( double const * a, int const nelem )
  {
    if ( gc_active == nullptr ) return GENERIC_CONTAINER_BAD_HEAD;
  ...
  int GC_set_vector_of_complex( double const * re, double const * im, int const nelem )
  {
    if ( gc_active == nullptr ) return GENERIC_CONTAINER_BAD_HEAD;
  ...
  int GC_set_vector_of_string( char const ** a, int const nelem )
  {
    if ( gc_active == nullptr ) return GENERIC_CONTAINER_BAD_HEAD;
  ...
  int GC_set_vector( int const nelem )
  {
    if ( gc_active == nullptr ) return GENERIC_CONTAINER_BAD_HEAD;
  ...
  int GC_push_vector_position( int const pos )
  {
    if ( gc_active == nullptr ) return GENERIC_CONTAINER_BAD_HEAD;
