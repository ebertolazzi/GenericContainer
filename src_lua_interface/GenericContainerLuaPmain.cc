...
*/
static void lstop( lua_State * L, lua_Debug * /* unused arg. */ )
{
...
static void print_version( char const ** messages )
{
  lua_writestring( LUA_COPYRIGHT, strlen( LUA_COPYRIGHT ) );
  lua_writeline();
  lua_writeline();
  for ( char const ** msg = messages; *msg != nullptr; ++msg ) l_message( progname, *msg );
  lua_writeline();
}
...
** print any results.
*/
static void doREPL( lua_State * L, char const prompt[] )
{
  int          status;
...
static int handle_script( lua_State * L, char ** argv )
{
  ...
  if ( strcmp( fname, "-" ) == 0 && strcmp( argv[-1], "--" ) != 0 ) fname = nullptr; /* stdin */
...
static int pmain( lua_State * L )
{
  ...
  char const ** messages = static_cast<char const **>( lua_touserdata( L, 3 ) );
  char const *  prompt   = lua_tostring( L, 4 );
  ...
  if (
    script < argc && /* execute main script (if there is one) */
...
namespace GC_namespace
{
  ...
  int LuaInterpreter::interactive( int argc, char const ** argv, char const ** messages, char const * prompt )
  {
    ...
    lua_pushlightuserdata( L, messages ); /* 4nd argument */
    lua_pushstring( L, prompt );          /* 5nd argument */
