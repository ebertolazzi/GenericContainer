...
#define DEF_UNIQUE( T ) std::unique_ptr<T> node( std::make_unique<T>( T{} ) )
  ...
  static std::unique_ptr<toml::node> GC_to_TOML( GenericContainer const & gc )
  {
    std::unique_ptr<toml::array> empty;
    switch ( gc.get_type() )
    {
      case GC_type::NOTYPE: return empty;
      ...
      {
        DEF_UNIQUE( toml::array );
        for ( auto vi : gc.get_vec_complex() ) node->as_array()->push_back( toml::array{ vi.real(), vi.imag() } );
        return node;
      }
  ...
  bool GenericContainer::to_toml( ostream_type & stream ) const
  {
    bool ok{ true };
    try
    {
