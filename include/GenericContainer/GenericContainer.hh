...
#define GC_WARNING(COND, MSG)                                                                      \
  ...
  {                                                                                                \
    cout                                                                                           \
      << "On line: " << __LINE__ << " file: " << __FILE__                                          \
      << " in GenericContainer\nWARNING: " << MSG << '\n';                                         \
  }
#endif
...
//!
//! \brief The `GenericContainer` class provides a flexible container for storing heterogeneous data
//! types.
//!
//! This class allows storage of various data types, including primitive types (e.g., integer,
//! floating-point), complex types, and containers like vectors and maps. It supports recursive
//! structures where elements of the container can themselves be other `GenericContainer` objects.
//!
...
//!
//! These capabilities make `GenericContainer` a highly versatile container for managing mixed-type
//! data in C++.
//!
//! ### Example:
//! \code
...
public:
  ...
  using vec_ulong_type = GC_namespace::vec_ulong_type;  //!< Alias for vector of unsigned long
                                                        //!< integers type
  using vec_real_type    = GC_namespace::vec_real_type; //!< Alias for vector of real numbers type
  using vec_complex_type = GC_namespace::vec_complex_type; //!< Alias for vector of complex numbers
                                                           //!< type
  ...
  using mat_complex_type = GC_namespace::mat_complex_type; //!< Alias for matrix of complex numbers
                                                           //!< type
...
private:
  ...
  //! Depending on the current data type stored in the container, different members of the union
  //! will be used.
  //!
...
public:
  ...
  //!
  //! This group of methods allows setting simple data types (e.g., pointer, boolean, integer, etc.)
  //! within a `GenericContainer`. Each method initializes the container to the specified type, sets
  //! the value, and returns a reference to the stored data.
  //! @{
  ...
  //! resets the container's type to `NOTYPE`, and returns a reference to the `GenericContainer`
  //! instance itself.
  //!
  ...
  //!
  //! \brief Extracts the keys of the map stored in the `GenericContainer` as a comma-separated
  //! string.
  //!
  //! If the container holds a map (`MAP` type), this method returns the map's keys as a single
  //! string, where each key is separated by a comma. If the container is not a map, an exception is
  //! thrown.
  //!
  ...
  //!
  //! This method initializes the `GenericContainer` to hold a boolean value, stores the provided
  //! value, and returns a reference to the stored boolean.
  //!
  ...
  //!
  //! This method initializes the `GenericContainer` to hold an integer value, stores the provided
  //! value, and returns a reference to the stored integer.
  //!
  ...
  //!
  //! This method initializes the `GenericContainer` to hold a long integer value, stores the
  //! provided value, and returns a reference to the stored long integer.
  //!
  ...
  //!
  //! This method initializes the `GenericContainer` to hold a floating-point value, stores the
  //! provided value, and returns a reference to the stored real value.
  //!
  ...
  //!
  //! This method initializes the `GenericContainer` to hold a complex number, stores the provided
  //! value, and returns a reference to the stored complex value.
  //!
  ...
  //!
  //! \brief Set the data type to `complex_type` and assign a complex value from real and imaginary
  //! parts.
  //!
  //! This method initializes the `GenericContainer` to hold a complex number. It takes two real
  //! numbers (representing the real and imaginary components), stores the complex number, and
  //! returns a reference to the stored complex value.
  //!
  ...
  //!
  //! This method initializes the `GenericContainer` to hold a string, allocates the necessary
  //! memory, stores the provided string value, and returns a reference to the stored string.
  //!
  ...
  //!
  //! This section includes methods for setting, allocating, and initializing vector and matrix data
  //! types within a `GenericContainer`. Each method allows for the creation of vectors or matrices,
  //! either by specifying the size or by copying from an existing vector or matrix.
  //! @{

  //! \brief Set the data to `vec_pointer_type`, allocate and initialize.
  //!
  //! This method allocates memory for a vector of pointers. If the specified size `sz` is greater
  //! than zero, the vector will be allocated with that size.
  //!
  ...
  //!
  //! Allocates memory for a vector of boolean values. If the specified size `sz` is greater than
  //! zero, the vector will be allocated with that size.
  //!
  ...
  //!
  //! Allocates memory for a vector of long integers. If the specified size `sz` is greater than
  //! zero, the vector will be allocated with that size.
  //!
  ...
  //!
  //! This method initializes the vector of long integers by copying data from the provided vector
  //! `v`.
  //!
  ...
  //!
  //! Allocates memory for a vector of `real_type` numbers. If the specified size `sz` is greater
  //! than zero, the vector will be allocated with that size.
  //!
  ...
  //!
  //! This method initializes the vector of `real_type` numbers by copying data from the provided
  //! vector `v`.
  //!
  ...
  //!
  //! Allocates memory for a vector of complex numbers. If the specified size `sz` is greater than
  //! zero, the vector will be allocated with that size.
  //!
  ...
  //!
  //! This method initializes the vector of complex numbers by copying data from the provided vector
  //! `v`.
  //!
  ...
  //!
  //! Allocates memory for a matrix of integers. If the specified number of rows `nr` and columns
  //! `nc` are greater than zero, the matrix will be allocated with size `nr` x `nc`.
  //!
  ...
  //!
  //! Allocates memory for a matrix of long integers. If the specified number of rows `nr` and
  //! columns `nc` are greater than zero, the matrix will be allocated with size `nr` x `nc`.
  //!
  //! \param[in] nr The number of rows in the matrix (default is 0).
  //! \param[in] nc The number of columns in the matrix (default is 0).
  ...
  //!
  //! This method initializes the matrix of long integers by copying data from the provided matrix
  //! `m`.
  //!
  ...
  //!
  //! Allocates memory for a matrix of `real_type` numbers. If the specified number of rows `nr` and
  //! columns `nc` are greater than zero, the matrix will be allocated with size `nr` x `nc`.
  //!
  //! \param[in] nr The number of rows in the matrix (default is 0).
  //! \param[in] nc The number of columns in the matrix (default is 0).
  ...
  //!
  //! This method initializes the matrix of `real_type` numbers by copying data from the provided
  //! matrix `m`.
  //!
  ...
  //!
  //! Allocates memory for a matrix of `complex_type` numbers. If the specified number of rows `nr`
  //! and columns `nc` are greater than zero, the matrix will be allocated with size `nr` x `nc`.
  //!
  //! \param[in] nr The number of rows in the matrix (default is 0).
  //! \param[in] nc The number of columns in the matrix (default is 0).
  ...
  //!
  //! This method initializes the matrix of complex numbers by copying data from the provided matrix
  //! `m`.
  //!
  ...
  //!
  //! This function sets the data type to `vector_type`, allocates an empty vector, and returns a
  //! reference to it. If the specified size `sz` is greater than 0, the vector is allocated with
  //! that size.
  //!
  ...
  //!
  //! This function sets the data type to `map_type`, allocates an empty map, and returns a
  //! reference to it.
  //!
...
//!
  ...
  //! \param[in] keys  The list of keys to search in the map.
  //! \param[in] where Optional context for error messages, indicating the position of the call.
  //! \return A reference to the vector of real numbers stored in the container for the first found
  //! key.
  //!
  ...
  //! \param[in] keys  The list of keys to search in the map.
  //! \param[in] where Optional context for error messages, indicating the position of the call.
  //! \return A reference to the vector of complex numbers stored in the container for the first
  //! found key.
  //!
  ...
  //! \code
  //! const GenericoContainer container;
  //! vec_complex_type const &complex_data = container.get_vec_complex("In function
  //! get_vec_complex");
  //! // Use complex_data...
  //! \endcode
  //!
  ...
  //! \code
  //! const GenericoContainer container;
  //! mat_complex_type const &complex_matrix = container.get_mat_complex("In function
  //! get_mat_complex");
  //! // Use complex_matrix...
  //! \endcode
  //!
  ...
  //! \code
  //! // Example usage of operator() with string key for const
  //! const GenericContainer &constItem = container("myKey", "Accessing const item with key
  //! 'myKey'");
  //! // Use constItem...
  //! \endcode
  //!
  ...
  //!
  //! Get a `GenericContainer` in the stored data by searching for a matching key from a vector of
  //! keys.
  //!
  ...
  //!
  //! Get a const `GenericContainer` in the stored data by searching for a matching key from a
  //! vector of keys.
  //!
  ...
  //! \param[in] a initializer data
  //!
  GenericContainer(string_type const &a) : m_data_type(GC_type::NOTYPE) { this->operator=(a); }
  ...
  //!
  void print_content_types(
    ostream_type &stream, string_type const &prefix = "", string_type const &indent = "    "
  ) const;
  ...
  //!
  string compare_content(GenericContainer const &gc, string const &from = "") const;
  ...
  GenericContainer &read_formatted_data(
    istream_type &stream, char const commentChars[] = "#%", char const delimiters[] = " \t"
  );
  ...
  //!
  GenericContainer &
  readFormattedData(istream_type &stream, char const commentChars[] = "#%", char const delimiters[] = " \t")
  {
  ...
  //!
  GenericContainer &
  read_formatted_data(char const fname[], char const commentChars[] = "#%", char const delimiters[] = " \t");

  //!
  //! \deprecated use `read_formatted_data`
  //!
  GenericContainer &
  readFormattedData(char const fname[], char const commentChars[] = "#%", char const delimiters[] = " \t")
  {
  ...
  //!                     Default are `#` and `%`
  //! \param delimiters   characters used as delimiter for headers
  //! \param ptr_pars     pointer to a `GenericContainer` which store poarameter parsed in the
  ...
  //!                     Default are `#` and `%`
  //! \param delimiters   characters used as delimiter for headers
  //! \param ptr_pars     pointer to a `GenericContainer` which store the parameter parsed in the
  //! comment part of the file
  //!
  GenericContainer &read_formatted_data2(
    char const fname[],
  ...
  //! The buffer must have enough space to store the serialized data (use \ref mem_size() to
  ...
  //! \return The number of bytes written into the buffer, or -1 if an error occurs (e.g.,
...
//!
void write_table(
  vec_string_type const &headers, vector_type const &data, ostream_type &stream, char delimiter = '\t'
);
...
//!
void write_table(
  vec_string_type const &headers, mat_real_type const &data, ostream_type &stream, char delimiter = '\t'
);
...
//!
inline void
writeTableFormatted(vec_string_type const &headers, vector_type const &data, ostream_type &stream)
{
...
//!
inline void
writeTableFormatted(vec_string_type const &headers, mat_real_type const &data, ostream_type &stream)
{
  write_table_formatted(headers, data, stream);
}
